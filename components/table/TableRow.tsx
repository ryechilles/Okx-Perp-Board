...
export function TableRow({
  ...
  const renderCell = (key: ColumnKey) => {
    ...
    const baseClass = `px-1 py-1.5 text-[13px] whitespace-nowrap ${alignClass} ${isFixed ? 'bg-white group-hover:bg-gray-50' : ''}`;
    ...
    switch (key) {
      case 'favorite':
        return (
          <td
            key={key}
            className={`py-1.5 text-center ${isFixed ? 'bg-white group-hover:bg-gray-50' : ''}`}
