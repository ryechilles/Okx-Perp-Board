 ...
 *   rightContent={
 *     <>
 *       <span className="text-[11px] text-gray-500">{formatPrice(price)}</span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        {rank !== undefined && (
          <span className="text-[11px] text-gray-400 w-4">{rank}</span>
        )}
        <TokenAvatar symbol={symbol} logo={logo} />
        <span className="text-[12px] font-medium text-gray-900">{symbol}</span>
      </div>
...
export function TokenRowSkeleton({
  ...
  return (
    <div className="flex items-center justify-between py-1.5">
      <div className="flex items-center gap-2">
        {showRank && <span className="text-[11px] text-gray-400 w-4">-</span>}
        <div className="w-5 h-5 rounded-full bg-gray-200 animate-pulse" />
        <div className="w-10 h-3 bg-gray-200 rounded animate-pulse" />
      </div>
      <div className="flex items-center gap-3">
        {Array.from({ length: columns }).map((_, i) => (
          <div key={i} className="w-12 h-3 bg-gray-200 rounded animate-pulse" />
        ))}
      </div>
    </div>
  );
