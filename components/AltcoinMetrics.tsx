...
export function AltcoinMetrics({ tickers, rsiData, marketCapData, onTokenClick, onTopNClick }: AltcoinMetricsProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row items-start gap-4 mb-4 overflow-x-auto pb-2">
      {/* Top Gainers Card */}
      <div
            ...
            {!isLoading && getPerformanceSummary() && (
              <>
                ...
                <div className="text-[11px]">
                  <span className="text-gray-400">{getPerformanceSummary()?.altStatus}  {getPerformanceSummary()?.btcStatus}</span>
                  <span className="text-gray-500 font-medium ml-2">{getPerformanceSummary()?.summary}</span>
                </div>
              </>
            )}
          </div>
